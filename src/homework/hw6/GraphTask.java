...
//        testComplementEdgeCountwithIncorrectCount(6, 9);
    ...
    class Graph {
         ...
         *
         * @param g - the randomly created simple graph
         */
        public void createComplementGraphforSimpleGraph(Graph g) {
            ...
            for (i = 0; i < vertexcount; i++) {
                for (j = 0; j < vertexcount; j++) {
                    if (i == j)
                        continue;
                    if (graphconnection[i][j] == 0 || graphconnection[j][i] == 0) {
                        Vertex vi = vertarray[i];
                        Vertex vj = vertarray[j];
                        createArc("a" + vi.toString() + "_" + vj.toString(), vi, vj);
                        graphconnection[i][j] = 1;
                        createArc("a" + vj.toString() + "_" + vi.toString(), vj, vi);
                        graphconnection[j][i] = 1;
                    }
                }
            }
        }
...
//    }
     ...
     * @param res adjacency matrix to be printed
     */
    private void printAdjMatrix(int[][] res) {
